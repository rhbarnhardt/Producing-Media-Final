...
(function(){
    ...
    var colors = ['#F77F22', '#541796', '#3E28D4', '#F63C25', '#00D4BF']
    ...
    function initText() {
        text = new createjs.Text("t", "80px 'Source Sans Pro'", "#545");
    ...
    function initCircles() {
        ...
        for(var i=0; i<3000; i++) {
            ...
            circle.alpha = alpha;
            circle.radius = r;
            circle.graphics.beginFill(color).drawCircle(10, 10, r);
            circle.x = x;
            circle.y = y;
            circles.push(circle);
    ...
    function tweenCircle(c, dir) {
        ...
        if(dir == 'in') {
            c.tween = TweenLite.to(c, 3, {x: c.originX, y: c.originY, ease:Quad.easeInOut, alpha: 1, radius: 9, scaleX: .3, scaleY: .5, onComplete: function() {
                c.movement = 'jiggle';
                tweenCircle(c);
            }});
        } else if(dir == 'out') {
            c.tween = TweenLite.to(c, 0.8, {x: window.innerWidth*Math.random(), y: window.innerHeight*Math.random(), ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.5, scaleX: 10, scaleY:10, onComplete: function() {
                c.movement = 'float';
                tweenCircle(c);
            }});
        } else {
            if(c.movement == 'float') {
                c.tween = TweenLite.to(c, 5 + Math.random()*3.5, {x: c.x + -100+Math.random()*200, y: c.y + -100+Math.random()*200, ease:Quad.easeInOut, alpha: 0.02 + Math.random()*0.5,
                    onComplete: function() {
                        tweenCircle(c);
                    }});
            } else {
                c.tween = TweenLite.to(c, 0.5, {x: c.originX + Math.random()*3, y: c.originY + Math.random()*3, ease:Quad.easeInOut,
                    onComplete: function() {
                        tweenCircle(c);
                    }});
            }
    ...
    function explode() {
        ...
        if(textPixels.length < circles.length) {
            for(var j = textPixels.length; j<circles.length; j++) {
                circles[j].tween = TweenLite.to(circles[j], 100, {alpha: 1});
            }
        }
    }
